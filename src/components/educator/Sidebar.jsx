...
const Sidebar = () => {
    ...
    const menuItems = [
        ...
        { name: 'Student Enrolled', path: '/educator/student-enrolled', icon: assets.person_tick_icon }, // Fixed typo here
    ]
    
    return isEducator && (
        <div className='ms:w-64 w-66 border-r min-h-screen text-base border-gray-500 py-3 flex flex-col'>
            {menuItems.map((item) => (
                <NavLink
                    to={item.path}
                    key={item.name}
